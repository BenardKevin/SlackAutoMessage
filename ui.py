...
def sidebar_inputs(defaults):
    with st.sidebar:
        ...
        course_name = st.text_input("Nom du cours", value=defaults.get("course_name", ""))
    return channel_id, date, course_name

def message_inputs(defaults):
    messages = []
    with st.expander("Modifier les messages à programmer", expanded=False):
        st.header("Messages à programmer")
        for i, msg in enumerate(defaults):
            col1, col2 = st.columns([1, 5])
            with col1:
                time = st.text_input(f"Heure {i+1}", value=msg["time"], key=f"time_{i}")
            with col2:
                text = st.text_area(f"Contenu {i+1}", value=msg["text"], height=100, key=f"text_{i}")
            messages.append({"time": time, "text": text})
    return messages

def preview_messages(messages, course_name):
    st.markdown("### Aperçu des messages personnalisés")
    for i, msg in enumerate(messages):
        formatted = msg["text"].replace("[Nom du cours]", course_name)
